...
const controlCLWI = async function () {
	try {
		...
		err.message === 'User denied Geolocation' ? pageMainContainer.renderError((message = `${err.message}: Please Allow location from your browser! and reload the page`)) : pageMainContainer.renderError(message);
	}
};
